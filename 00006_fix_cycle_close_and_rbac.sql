...
begin
  if tg_op = 'INSERT' then
    -- A brand-new flat has no admin yet, so create_flat's own INSERT of the founding
    -- member (role='admin') must be allowed to bootstrap. Once the flat has at least one
    -- active member, only an existing admin may assign admin/manager to anyone else.
    if new.role <> 'member'
       and not private.is_flat_admin(new.flat_id)
       and exists (select 1 from public.flat_members where flat_id = new.flat_id and status = 'active')
    then
