...
export async function GET(request: NextRequest) {
  ...
  const { data: membership, error: membershipError } = await supabase.from('flat_members').select('flat_id').eq('user_id', user.id).eq('status', 'active').maybeSingle()
  if (membershipError) return NextResponse.json({ error: membershipError.message }, { status: 500 })
  if (!membership) return NextResponse.json({ error: 'No active flat' }, { status: 403 })

  const { data: cycle, error: cycleError } = await supabase.from('cycles').select('id,flat_id,start_date,end_date,status').eq('id', cycleId).eq('flat_id', membership.flat_id).maybeSingle()
  if (cycleError) return NextResponse.json({ error: cycleError.message }, { status: 500 })
  if (!cycle) return NextResponse.json({ error: 'Cycle not found' }, { status: 404 })
  if (cycle.status !== 'closed') return NextResponse.json({ error: 'Settlement report is available after the cycle is closed' }, { status: 409 })

  const { data: settlements, error: settlementError } = await supabase.from('settlements').select('total_meals,meal_cost,total_contribution,balance,opening_balance,user_id').eq('cycle_id', cycle.id)
