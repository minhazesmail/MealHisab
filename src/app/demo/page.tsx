...
export default function DemoPage() {
  ...
  return (
    <main className="min-h-screen bg-canvas text-main">
      ...
      <div className="fixed bottom-4 left-1/2 z-50 flex w-[calc(100%-2rem)] max-w-xl -translate-x-1/2 items-center justify-between gap-3 rounded-2xl border border-line-strong bg-surface/95 px-4 py-3 shadow-2xl backdrop-blur-xl"><div className="min-w-0"><p className="text-xs font-bold text-brand-green">{locale === 'bn' ? 'বাস্তব অ্যাপ ব্যবহার করতে প্রস্তুত?' : 'Ready to use the real app?'}</p><p className="truncate text-[11px] text-muted">{locale === 'bn' ? 'ম্যানেজার ৳49/মাস · সদস্য ফ্রি' : 'Manager: ৳49/month · Members: free'}</p></div><div className="flex shrink-0 gap-2"><Link href="/register/manager" className="rounded-xl bg-brand-green px-3 py-2 text-[11px] font-bold text-black">{locale === 'bn' ? 'ম্যানেজার' : 'Manager'}</Link><Link href="/join" className="rounded-xl border border-line-strong bg-surface-2 px-3 py-2 text-[11px] font-bold text-brand-green">{locale === 'bn' ? 'ফ্রি যোগ দিন' : 'Join free'}</Link></div></div>
