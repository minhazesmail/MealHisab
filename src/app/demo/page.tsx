...
export default function DemoPage() {
  ...
  function addContribution() {
    ...
    setContributionNote('')
    notify(`${money(amount)} contribution recorded.`)
  }
  ...
  return (
    <main className="min-h-screen bg-canvas text-main">
      <header className="sticky top-0 z-30 border-b border-line bg-surface/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6">
          ...
          <div className="flex items-center gap-2">
            <LanguageToggle />
            <button type="button" onClick={resetDemo} className="hidden items-center gap-2 rounded-xl border border-line bg-surface-2 px-3 py-2 text-xs font-semibold sm:inline-flex"><RotateCcw size={14} /> Reset</button>
            <Link href="/account-type" className="rounded-xl bg-brand-green px-4 py-2 text-xs font-bold text-black">Start with your flat</Link>
          </div>
        </div>
      </header>

      <div className="mx-auto max-w-7xl px-4 py-5 sm:px-6">
        <div className="rounded-2xl border border-brand-green/25 bg-surface px-4 py-4 shadow-soft">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-start gap-3"><div className="rounded-xl bg-brand-green/10 p-2.5 text-brand-green"><CreditCard size={18} /></div><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-brand-green">Manager Plan</p><p className="mt-1 text-sm font-bold">৳99/month · One flat · 10 invite codes/month</p><p className="text-xs text-muted">Members join free with a valid Flat Code. This is an interactive sample-only demo.</p></div></div>
      ...
      <div className="mx-auto grid max-w-7xl gap-6 px-4 pb-24 sm:px-6 lg:grid-cols-[230px_1fr]">
        <aside className="rounded-3xl border border-line bg-surface p-3 shadow-soft lg:sticky lg:top-24 lg:h-fit">
          <div className="mb-3 rounded-2xl border border-line bg-surface-2 px-3 py-3"><p className="text-xs font-bold">Mirpur Mess</p><p className="mt-1 text-[11px] text-muted">01–31 Aug 2026 · Auto date: {assignedDate}</p></div>
          <nav className="grid grid-cols-2 gap-1 lg:grid-cols-1">
            {tabs.map((tab) => {
              const Icon = tab.icon
              return <button type="button" key={tab.id} onClick={() => setActive(tab.id)} className={`flex items-center gap-2 rounded-xl px-3 py-2.5 text-left text-xs font-semibold ${active === tab.id ? 'border border-line-strong bg-surface-3 text-brand-green shadow-glow' : 'text-muted hover:bg-surface-2 hover:text-main'}`}><Icon size={15} />{tab.label}</button>
            })}
          </nav>
        ...
        <section className="min-w-0">
          <div className="mb-5"><p className="text-xs font-bold uppercase tracking-[0.18em] text-brand-green">Interactive product tour</p><h1 className="mt-1 text-2xl font-black sm:text-3xl">Try MealHisab before you sign up.</h1><p className="mt-1 max-w-2xl text-sm text-muted">Play with meals, expenses, contributions, closed days and settlements. Everything is sample-only.</p></div>
          ...
          {active === 'reports' && <div className="rounded-3xl border border-line bg-surface p-5 shadow-soft"><div className="flex items-center gap-2"><Check size={16} className="text-brand-green" /><h2 className="text-lg font-bold">Settlement preview</h2></div>{members.map((member) => { const balance = settlementFor(member); return <div key={member.id} className="flex items-center justify-between border-t border-line py-3"><div><p className="text-sm font-semibold">{member.name}</p><p className="text-xs text-muted">{member.meals} meals · {money(member.contribution)} contributed</p></div><p className={`font-black ${balance >= 0 ? 'text-brand-green' : 'text-danger'}`}>{balance >= 0 ? 'Receivable' : 'Payable'} {money(Math.abs(balance))}</p></div>})}</div>}
        </section>
      </div>
