...
export default async function ManagerSubscribePage() {
  ...
  return <main className="min-h-screen bg-canvas px-4 py-10 text-main"><div className="mx-auto max-w-2xl"><div className="mb-6"><p className="text-xs font-bold uppercase tracking-[0.2em] text-brand-green">Manager Plan</p><h1 className="mt-2 text-3xl font-black">৳49/month</h1><p className="mt-2 text-sm leading-6 text-muted">One flat, up to 10 member invite codes per month, meal accounting, settlement reports, and member management.</p></div><ManualPaymentCard active={active} periodEnd={sub?.current_period_end ?? null} paymentStatus={payment?.status === 'pending' ? 'submitted' : null} bkashNumber={bkashNumber} nagadNumber={nagadNumber} rocketNumber={rocketNumber}/>{active && <Link href="/onboarding/create-flat" className="btn-primary mt-5 flex w-full justify-center">Continue to create your flat</Link>}</div></main>
}
