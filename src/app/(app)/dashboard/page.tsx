...
export default async function DashboardPage() {
  ...
  const { data: membership, error: membershipError } = await supabase.from('flat_members').select('flat_id,role').eq('user_id', user.id).eq('status', 'active').maybeSingle()
  if (membershipError) return <div className="card text-sm text-red-600">Could not load your membership.</div>
  if (!membership) redirect('/onboarding')

  const { data: flat, error: flatError } = await supabase.from('flats').select('id,name,invite_code,meal_policy').eq('id', membership.flat_id).maybeSingle()
  ...
  const { data: cycle, error: cycleError } = await supabase.from('cycles').select('id,flat_id,start_date,end_date,status').eq('flat_id', membership.flat_id).eq('status', 'open').order('start_date', { ascending: false }).limit(1).maybeSingle()
