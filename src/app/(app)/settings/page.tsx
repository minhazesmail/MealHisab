...
export default async function SettingsPage() {
  ...
  const { data: flat } = await s.from('flats').select('*').eq('id', m.flat_id).maybeSingle()
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold">Flat settings</h1>
        <p className="text-sm text-slate-500">Share the invite code and control the accounting cycle.</p>
      </div>
      ...
      <section className="card">
        ...
        <div className="space-y-3">
          {typedMembers.map(x => (
            <div key={x.user_id} className="flex items-center justify-between border-b pb-3 text-sm">
              <div><div className="font-medium">{x.profiles?.[0]?.full_name ?? 'Member'}</div><div className="text-slate-500">{x.status} · {x.role}</div></div>
              <div className="text-slate-500">Joined {x.joined_at}</div>
            </div>
          ))}
        </div>
      </section>
      {(m.role === 'admin' || m.role === 'manager') && cycle && (
        <section className="card flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div><h2 className="font-semibold">Close current cycle</h2><p className="text-sm text-slate-500">This snapshots balances, closes the cycle and creates the next one in one transaction.</p></div>
