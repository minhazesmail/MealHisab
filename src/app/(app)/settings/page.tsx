...
export default async function SettingsPage() {
  ...
  const {data:inviteCodes}=await s.from('invite_codes').select('id,code,created_at,revoked_at').eq('flat_id',m.flat_id).order('created_at',{ascending:false}).limit(20)
  ...
  const inviteRows=(inviteCodes??[]).map((x:any)=>({id:x.id,code:x.code,createdAt:x.created_at,revokedAt:x.revoked_at}))
