...
export default async function SettingsPage() {
  ...
  const {data:inviteCodes}=await s.from('invite_codes').select('id,code,status,max_uses,used_count,created_month,created_at,revoked_at,expires_at,used_at').eq('flat_id',m.flat_id).order('created_at',{ascending:false}).limit(20)
  ...
  const inviteRows=(inviteCodes??[]).map((x:any)=>({id:x.id,code:x.code,status:x.status as 'active'|'used'|'revoked'|'expired',maxUses:Number(x.max_uses),usedCount:Number(x.used_count),createdMonth:x.created_month,createdAt:x.created_at,revokedAt:x.revoked_at,expiresAt:x.expires_at,usedAt:x.used_at}))
