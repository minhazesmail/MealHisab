...
export default async function MealsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <p className="text-sm text-slate-500">{mealDate}</p>
        <h1 className="text-2xl font-bold">Today&apos;s meals</h1>
        <p className="mt-1 text-sm text-slate-500">
          {flat.meal_policy === 'opt_out'
            ? 'Meals start counted; tap Skip when you will not eat.'
            : 'Tap I ate when you consume a meal.'}
        </p>
      </div>
      <MealTracker
