...
export function GuestMealForm({ cycleId, date, approvalRequired }: { cycleId: string; date: string; approvalRequired: boolean }) {
  ...
  return <form className="card space-y-3" onSubmit={(e) => { e.preventDefault(); setError(''); const f=new FormData(e.currentTarget); start(async()=>{try{await recordGuestMeal({cycleId,mealDate:date,mealType:String(f.get('mealType')) as 'lunch'|'dinner',guestCount:Number(f.get('guestCount')),note:String(f.get('note')||'')}); toast.success(approvalRequired?'Guest meal submitted for manager approval':'Guest meal recorded'); e.currentTarget.reset(); router.refresh()}catch(err){const m=err instanceof Error?err.message:'Could not save guest meal';setError(m);toast.error(m)}})}}>
...
export function GuestMealApprovalList({ rows }: { rows: Array<{ id: string; name: string; date: string; mealType: string; count: number; status: string }> }) {
  const router=useRouter(); const [pendingId,start]=useState<string|null>(null)
  if(!rows.length)return null
  return <section className="card space-y-3"><div><h2 className="font-semibold">Guest approvals</h2><p className="mt-1 text-sm text-muted">Pending guest meals must be approved before they affect accounting.</p></div>{rows.map(row=><div key={row.id} className="flex flex-col gap-3 rounded-2xl border border-line bg-surface-2 p-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="font-medium">{row.name}</div><div className="text-sm text-muted">{row.date} · {row.mealType} · {row.count} guest{row.count===1?'':'s'}</div></div><div className="flex gap-2"><button type="button" className="btn-primary text-xs" disabled={pendingId===row.id} onClick={async()=>{startTransition?null:null;}} >Approve</button></div></div>)}</section>
}
