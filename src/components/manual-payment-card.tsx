...
export function ManualPaymentCard({ active, periodEnd, paymentStatus, bkashNumber, nagadNumber, rocketNumber }: Props) {
  ...
  return <section className="card border-brand-green/30 bg-surface-2">
    <div className="mb-5"><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-brand-green">Manager Plan</p><h2 className="mt-1 text-lg font-semibold">৳99 / month</h2><p className="mt-1 text-sm leading-6 text-muted">{active ? `Your Manager Plan is active${periodEnd ? ` until ${new Date(periodEnd).toLocaleDateString('en-BD')}` : ''}. Submit another ৳99 payment now to extend it by 30 days when approved.` : 'Send exactly ৳99 to one of the payment numbers below, then submit your transaction details for manual verification.'}</p></div>
    ...
    <form action={submit} className="mt-5 space-y-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs text-muted">Approval is manual. Keep your payment receipt until the request is approved.</p><button className="btn-primary" disabled={pending}>{pending ? 'Submitting…' : active ? 'Submit renewal payment' : 'Submit payment proof'}</button></div>
