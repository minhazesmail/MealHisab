...
export function ManualPaymentCard({ active, periodEnd, paymentStatus }: { active: boolean; periodEnd: string | null; paymentStatus?: string | null }) {
  ...
  function submit(formData: FormData) {
    ...
    startTransition(async () => {
      try {
        await submitManualManagerPayment(formData)
      } catch (err) {
  ...
  return <section className="card border-brand-green/30 bg-surface-2">
    <div className="mb-5">
      <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-brand-green">Manager Plan</p>
      <h2 className="mt-1 text-lg font-semibold">Pay ৳99 / month</h2>
      <p className="mt-1 text-sm leading-6 text-muted">Send exactly ৳99 to one of the payment numbers below, then submit your transaction details for manual verification.</p>
    </div>
    ...
    <div className="grid gap-3 sm:grid-cols-3">
      {[
        ...
        ['Rocket', 'YOUR-ROCKET-NUMBER'],
      ].map(([name, number]) => <div key={name} className="rounded-2xl border border-line bg-surface-3 p-3"><div className="text-xs font-bold text-main">{name}</div><div className="mt-1 font-mono text-sm text-brand-green">{number}</div></div>)}
    </div>

    <form action={submit} className="mt-5 space-y-4">
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="block text-sm font-semibold text-main">Payment method<select name="payment_method" className="input mt-1.5" required defaultValue="bkash"><option value="bkash">bKash</option><option value="nagad">Nagad</option><option value="rocket">Rocket</option></select></label>
        <label className="block text-sm font-semibold text-main">Sender number<input name="sender_number" className="input mt-1.5" inputMode="tel" placeholder="01XXXXXXXXX" required/></label>
      </div>
