...
const navItems: NavItem[] = [
  ...
  ['nav.meals', '/meals', Utensils],
  ['nav.calendar', '/calendar', CalendarDays],
  ['nav.expenses', '/expenses', CircleDollarSign],
  ['nav.contributions', '/contributions', WalletCards],
  ['nav.settlements', '/settlements', BarChart3],
        ...
        {navItems.map(({ key, href, icon: Icon }) => {
          ...
          return (
            <Link
              key={href}
              href={href}
              className={`group flex items-center gap-3 rounded-xl border px-3 py-3 text-sm font-semibold transition ${
                active
                  ? 'border-line-strong bg-surface-3 text-brand-green shadow-glow'
                  : 'border-transparent text-muted hover:border-line hover:bg-surface-2 hover:text-main'
              }`}
            >
              <Icon size={18} strokeWidth={active ? 2.3 : 2} />
              <span className="flex-1">{t(key)}</span>
              <ChevronRight size={14} className={`transition ${active ? 'opacity-100' : 'opacity-0 group-hover:opacity-60'}`} />
            </Link>
          )
        })}
      </nav>
      <div className="border-t border-line px-5 py-4 text-[11px] text-muted">MealHisab BD</div>
    </aside>
  )
}
...
export function MobileNav() {
  ...
  return (
    <>
      ...
      {moreOpen && (
        <div className="fixed inset-0 z-30 bg-black/50 backdrop-blur-sm lg:hidden" onClick={() => setMoreOpen(false)}>
          <div className="absolute inset-x-0 bottom-[calc(env(safe-area-inset-bottom)+4.8rem)] mx-auto max-w-lg px-3" onClick={(event) => event.stopPropagation()}>
            <div className="rounded-2xl border border-line-strong bg-surface p-3 shadow-2xl">
              <div className="mb-2 px-2 text-[10px] font-bold uppercase tracking-[0.2em] text-muted">More</div>
              <div className="grid grid-cols-2 gap-2">
                {mobileMore.map(({ key, href, icon: Icon }) => {
                  const active = isActive(pathname, href)
                  return (
                    <Link key={href} href={href} onClick={() => setMoreOpen(false)} className={`flex items-center gap-3 rounded-xl border px-3 py-3 text-sm font-semibold ${active ? 'border-line-strong bg-surface-3 text-brand-green' : 'border-line bg-surface-2 text-main'}`}>
                      <Icon size={18} />
                      <span>{t(key)}</span>
                    </Link>
                  )
