...
returns boolean language sql stable security definer set search_path = '' as $$
  select exists (
    ...
    where fm.flat_id = p_flat_id and fm.user_id = (select auth.uid()) and fm.status = 'active'
  );
$$;
...
returns boolean language sql stable security definer set search_path = '' as $$
  select exists (
    select 1 from public.flat_members fm
    where fm.flat_id = p_flat_id and fm.user_id = (select auth.uid()) and fm.status = 'active' and fm.role in ('admin','manager')
  );
$$;
...
returns boolean language sql stable security definer set search_path = '' as $$
  select exists (
    select 1 from public.flat_members fm
    where fm.flat_id = p_flat_id and fm.user_id = (select auth.uid()) and fm.status = 'active' and fm.role = 'admin'
  );
$$;
...
create policy meal_insert_own_or_manager on public.meal_logs for insert to authenticated with check (private.is_flat_member(flat_id) and (user_id = (select auth.uid()) or private.is_flat_manager(flat_id)) and exists (select 1 from public.cycles c where c.id = cycle_id and c.flat_id = flat_id and c.status = 'open'));
create policy meal_update_own_or_manager on public.meal_logs for update to authenticated using (private.is_flat_member(flat_id) and (user_id = (select auth.uid()) or private.is_flat_manager(flat_id))) with check (private.is_flat_member(flat_id) and (user_id = (select auth.uid()) or private.is_flat_manager(flat_id)));
...
create policy contributions_insert_own_or_manager on public.contributions for insert to authenticated with check (private.is_flat_member(flat_id) and (user_id = (select auth.uid()) or private.is_flat_manager(flat_id)) and exists (select 1 from public.cycles c where c.id = cycle_id and c.flat_id = flat_id and c.status = 'open'));
