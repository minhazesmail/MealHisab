...
create index if not exists idx_member_leave_cycle_user_dates
  on public.member_leave(cycle_id, user_id, start_date, end_date);
...
create policy member_leave_select on public.member_leave for select to authenticated using (
...
create policy member_leave_insert_self_or_manager on public.member_leave for insert to authenticated with check (
  ...
  and exists (
    ...
    where c.id = cycle_id
      and (
        private.is_flat_manager(c.flat_id)
        or exists (
          select 1 from public.flat_members fm
          where fm.flat_id = c.flat_id and fm.user_id = auth.uid() and fm.status = 'active'
        )
      )
  )
);
...
create policy member_leave_update_manager on public.member_leave for update to authenticated using (
  exists (select 1 from public.cycles c where c.id = cycle_id and private.is_flat_manager(c.flat_id))
) with check (
  exists (select 1 from public.cycles c where c.id = cycle_id and private.is_flat_manager(c.flat_id))
);

drop function if exists public.effective_meal_count(uuid,uuid);
create function public.effective_meal_count(p_cycle_id uuid, p_user_id uuid)
returns integer
language sql
stable
security definer
...
as $$
  ...
  cm as (
    select cycle_id, user_id, active_from, active_to
      from public.cycle_members
     where cycle_id = p_cycle_id and user_id = p_user_id
  ),
  days as (
    select gs.d::date as service_date, ctx.meal_policy
      from ctx
      join cm on cm.cycle_id = ctx.cycle_id
  ...
  daily as (
    select d.service_date,
      coalesce(max(ml.count) filter (where ml.meal_type='lunch'),
        case when d.meal_policy='opt_out' and not exists (
          select 1 from public.member_leave l
           where l.cycle_id=p_cycle_id and l.user_id=p_user_id and l.status='approved'
             and d.service_date between l.start_date and l.end_date
        ) then 1 else 0 end)
      ...
      left join public.meal_logs ml
        on ml.cycle_id=p_cycle_id and ml.user_id=p_user_id and ml.date=d.service_date
...
create or replace function public.request_member_leave(
  p_cycle_id uuid,
  p_start_date date,
  p_end_date date,
...
begin
  ...
  if not exists (select 1 from public.flat_members where flat_id=v_flat and user_id=v_user and status='active') then
    raise exception 'not_active_member';
  end if;
  if p_end_date < p_start_date then raise exception 'invalid_vacation_dates'; end if;
  if p_start_date < (select start_date from public.cycles where id=p_cycle_id)
     or p_end_date > (select end_date from public.cycles where id=p_cycle_id) then
    raise exception 'vacation_outside_cycle';
  end if;
  v_days := p_end_date - p_start_date + 1;
  v_status := case when v_days <= 7 then 'approved' else 'pending' end;
  insert into public.member_leave(cycle_id,user_id,start_date,end_date,reason,created_by,approved_by,approved_at,status)
  values(p_cycle_id,v_user,p_start_date,p_end_date,coalesce(nullif(trim(p_reason),''),'Vacation'),v_user,
    case when v_status='approved' then v_user else null end,
    case when v_status='approved' then now() else null end,
    v_status)
  on conflict(cycle_id,user_id,start_date,end_date) do update set reason=excluded.reason
  returning id into v_id;
  return v_id;
end;
$$;

create or replace function public.manager_set_member_leave(
  p_cycle_id uuid,
  p_user_id uuid,
  p_start_date date,
  p_end_date date,
...
begin
  ...
  if p_end_date < p_start_date then raise exception 'invalid_vacation_dates'; end if;
  if p_start_date < (select start_date from public.cycles where id=p_cycle_id)
     or p_end_date > (select end_date from public.cycles where id=p_cycle_id) then raise exception 'vacation_outside_cycle'; end if;
  ...
  insert into public.member_leave(cycle_id,user_id,start_date,end_date,reason,created_by,approved_by,approved_at,status)
  values(p_cycle_id,p_user_id,p_start_date,p_end_date,coalesce(nullif(trim(p_reason),''),'Vacation'),v_user,v_user,now(),'approved')
  on conflict(cycle_id,p_user_id,start_date,end_date) do update set reason=excluded.reason,approved_by=v_user,approved_at=now(),status='approved'
  returning id into v_id;
  return v_id;
end;
$$;
...
begin
  ...
  select c.flat_id into v_flat
    from public.member_leave ml join public.cycles c on c.id=ml.cycle_id
   where ml.id=p_leave_id and ml.status='pending';
  if v_flat is null or not private.is_flat_manager(v_flat) then raise exception 'forbidden'; end if;
...
declare
  v_user uuid := auth.uid();
  r public.member_leave;
  v_flat uuid;
begin
  if v_user is null then raise exception 'not_authenticated'; end if;
  select ml.* into r from public.member_leave ml where ml.id=p_leave_id and ml.status in ('pending','approved') for update;
  if r.id is null then raise exception 'vacation_not_found'; end if;
  select flat_id into v_flat from public.cycles where id=r.cycle_id;
  if r.user_id <> v_user and not private.is_flat_manager(v_flat) then raise exception 'forbidden'; end if;
