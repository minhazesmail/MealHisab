...
describe('MealHisab accounting', () => {
  it('uses implicit opt-out lunch/dinner meals', () => {
    const overrides = new Map<string, MealOverride>()
    ...
    expect(
      effectiveMealCount(
        ...
        '2026-08-19',
        'lunch',
        new Map<string, MealOverride>([
          ['u1:2026-08-19:lunch', { userId: 'u1', date: '2026-08-19', mealType: 'lunch', count: 0 }],
        ]),
      ),
    ).toBe(0)
  })

  it('uses zero for missing opt-in meals', () => {
    expect(effectiveMealCount('opt_in', 'u1', '2026-08-19', 'dinner', new Map<string, MealOverride>())).toBe(0)
  })

  it('protects zero meal rate', () => {
    expect(calculateMealRate(1000, 0)).toBe(0)
  })
  ...
  it('calculates member cost and carry-forward balance', () => {
    ...
    expect(result.mealCost).toBe(1600)
  ...
  it('attributes extra meals to the host member', () => {
    ...
    const logs = new Map<string, MealOverride>([
      ['u1:2026-08-19:extra', { userId: 'u1', date: '2026-08-19', mealType: 'extra', count: 2 }],
    ])
